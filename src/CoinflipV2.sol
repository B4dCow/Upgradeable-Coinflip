...
contract CoinflipV2 is Initializable, OwnableUpgradeable, UUPSUpgradeable{
    ...
    function seedRotation(string memory NewSeed) public onlyOwner {
        ...
        //Takes care of the case when the number of rotation is bigger than the length
        NbRotation = NbRotation % SeedLength;
        ...
        // Applies a rotation of each byte of the string
        for (uint8 i=0;i < SeedLength;i++){
            if (i+NbRotation<SeedLength){
                RotatedSeed[i + NbRotation] = stringInBytes[i];
            } else {
                RotatedSeed[i + NbRotation - SeedLength] = stringInBytes[i];
            }
        }
